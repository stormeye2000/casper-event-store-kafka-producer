...
class KafkaEventBuilderTest {
    ...
    void buildKafkaEvent() {
        ...
        final Stream<Event> stream = EventService.usingPeer(source).readEvent(EventType.MAIN, EventTarget.RAW, new StringReader(dataTxt + "\n" + idTxt));
        final Optional<Event> first = stream.findFirst();

        assertThat(first.isPresent(), is(true));

        final String kafkaEvent = KafkaEventBuilder.buildKafkaEvent(first.get());
